...
(async function () {
    ...
    if (!$.isEmptyObject(requests)) { $('#charger-request').children().remove(); }
        else { $('#charger-request').children()[0].innerHTML = "<p>No pending booking requests!</p>"  }
...
const createConfirmationPopup = (choice, request) => {
    ...
    createPopup();
    createPopupHeader("h5", "" + choice + " request ?<br><b id='confirm-charger-name'>" + request.chargername + "</b>"
